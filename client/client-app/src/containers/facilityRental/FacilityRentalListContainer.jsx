...
const FacilityRentalListContainer = () => {
  ...
  const getFrList = async () => {
    const response = await frApi.frList({ pageNo, rows, searchType, keyword, order }, jwtSets.jwtToken);
  ...
  const initPage = async () => {
    const response = await frApi.pageInfo({ pageNo, rows, pageCount, totalCount, searchType, keyword }, jwtSets.jwtToken)
    const data = await response.data
    console.log(data)
    setPageInfo(data)
  }
