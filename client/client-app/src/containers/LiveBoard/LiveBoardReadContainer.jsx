...
const LiveBoardReadContainer = ({ no }) => {
  ...
  const purchaseticket = async () => {
    const response1 = await liveBoards.getTicketNum(boardNo, name, phone, ticketCount, jwtSets.jwtToken)
    ...
    if (data1 == 'SUCCESS') {
      ...
      IMP.request_pay({
        ...
        if (rsp.success) {
          ...
          console.log(rsp);
          //티켓 구매
          const response3 = await liveBoards.ticketPurchase(boardNo, name, phone, count, jwtSets.jwtToken);
    ...
    let timerId = setInterval(async () => {
      ...
      // 매진 체크
      // 등록 요청
      const response2 = await liveBoards.getTicketNum(boardNo, name, phone, ticketCount, jwtSets.jwtToken)
